...
exports.CompletTimeLoggingService = void 0;
...
class CompletTimeLoggingService {
    constructor() {
        const jiraConfig = (0, settings_1.getJiraConfig)();
        this.config = {
    ...
    async makeJiraRequest(endpoint, method = 'GET', data) {
    ...
    async findCTLProject() {
        ...
        try {
            ...
            const ctlProject = projects.find((project) => project.attributes.name.toLowerCase().includes('ctl') &&
     ...
     */
    async findServiceForProject(personId, projectId) {
        console.log('🛠️  Finding appropriate service for user and project...');
        try {
            // First check if user has logged time to this project before
            const timeEntriesResponse = await this.makeRequest(`/time_entries?filter[person_id]=${personId}&filter[project_id]=${projectId}&page[size]=10&include=service`);
            if (timeEntriesResponse.data.length > 0) {
                console.log(`   📊 Found ${timeEntriesResponse.data.length} previous time entries`);
                const serviceIds = new Set(timeEntriesResponse.data.map((entry) => entry.relationships.service.data.id));
                if (serviceIds.size === 1) {
                    const serviceId = Array.from(serviceIds)[0];
                    let serviceName = 'Unknown Service';
                    if (timeEntriesResponse.included) {
                        const serviceData = timeEntriesResponse.included.find((inc) => inc.type === 'services' && inc.id === serviceId);
                        if (serviceData) {
            ...
            const projectTimeEntriesResponse = await this.makeRequest(`/time_entries?filter[project_id]=${projectId}&page[size]=50&include=service`);
            if (projectTimeEntriesResponse.data.length > 0) {
                // Extract unique services used in this project
                const serviceIds = new Set(projectTimeEntriesResponse.data.map((entry) => entry.relationships.service.data.id));
     ...
     * Log time to Jira
     */
    async logTimeToJira(ticketId, timeMinutes) {
        const timeData = {
     ...
     */
    async logTimeToProductive(personId, projectId, serviceId, timeMinutes, description, jiraTicketId) {
     ...
     */
    async autoLogTime(jiraTicketId, timeMinutes, description) {
        ...
        try {
            ...
            if (result.jiraLogSuccess) {
                ...
                try {
                    ...
                    await this.logTimeToProductive(person.id, project.id, serviceResult.serviceId, timeMinutes, timeDescription, jiraTicketId);
...
exports.CompletTimeLoggingService = CompletTimeLoggingService;
...
//# sourceMappingURL=complete-time-logging.test.js.map