...
class ImprovedServiceDiscovery {
     ...
     */
    async getAuthenticatedUserViaMembership() {
     ...
     */
    async findServicesForFirstTimeUser(personId, projectId) {
     ...
     */
    async discoverServiceForProject(projectId) {
        ...
        try {
            ...
            const timeEntriesResponse = await this.makeRequest(`/time_entries?filter[person_id]=${person.id}&filter[project_id]=${projectId}&page[size]=10&include=service`);
            if (timeEntriesResponse.data.length > 0) {
                ...
                if (uniqueServiceIds.length === 1) {
                    const serviceId = uniqueServiceIds[0];
...
//# sourceMappingURL=improved-discovery.js.map