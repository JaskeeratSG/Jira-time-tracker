            ...
            if (projectKey) {
                ...
                if (!targetProject) {
                    targetProject = projects.find((p) => {
                        ...
                        return jiraProjectWords.some(jiraWord => jiraWord.length > 1 && productiveProjectWords.some((prodWord) => prodWord.includes(jiraWord) || jiraWord.includes(prodWord)));
                    });
                    if (targetProject) {
    ...
    async discoverBestService(credentials, personId, projectId) {
        ...
        try {
            ...
            if (userTimeEntriesResponse.data.data.length > 0) {
                ...
                const serviceDetails = userTimeEntriesResponse.data.included?.find((item) => item.type === 'services' && item.id === serviceId);
                if (serviceDetails) {
                    return {
